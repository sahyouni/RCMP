...
public class PoliceOfficer {
     ...
     * @return ticketList
     */
    public void getTicketList(int index) {
        if (index >= 0 && index < ticketList.size()) {
            ParkingTicket oneTicket = ticketList.get(index);
        } else {
    ...
    /**
     * A method to issue parking ticket New modification of adding each issued
     * ticket to the ticketList Not return anything
     */
    public ParkingTicket issueParkingTicket(ParkedCar car, ParkingMeter meter) {

        // it looks to me that this method should return a ParkingTicket for the
        // supplied car at the given meter
        // however if the car does not have a parking ticket, return null

        if (isParkingTimeExpired(car, meter) == false) {
            return null;
        }

        // issue a ticket and return it
        ParkingTicket parkingTicket = new ParkingTicket(officerName, officerBadgeNumber, car.getLicensePlateNumber(),
                calculateFine(car, meter));
    ...
    /**
     * A method to calculate the sum of fines of all the parkingTickets in the
     * collection return the sum
     */
    ...
    /**
     * A method to count the total amount of tickets issued to one license plate
     * number
     * 
     * @param inputLicense
     ...
     * 
     * @param inputLicense
     * @return an array of all the parking ticket objects issued to the car with the
     *         given license number
     */
    public ArrayList<ParkingTicket> getTicketArrayByLicenseNumber(String inputLicense) {
        ArrayList<ParkingTicket> ticketArrayByLicenseNumber = new ArrayList<ParkingTicket>();
        for (ParkingTicket parkingTicket : ticketList) {
            String license = parkingTicket.getCarLicensePlateNumber();
            if (inputLicense.equalsIgnoreCase(license)) {
                ticketArrayByLicenseNumber.add(parkingTicket);
            }
        }
    ...
    /**
     * A method to display the details of all the parking ticket objects in the
     * ticketList
     */
    ...
    /**
     * A method to delete all the parking tickets that issued to the car with the
     * provided plate number
     * 
     * @param inputLicense
